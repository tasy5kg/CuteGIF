...
class CustomMenuItemView(context: Context, attrs: AttributeSet) :
  ...
  init {
    context.theme.obtainStyledAttributes(
      ...
      binding.mtvTitle.text = getString(R.styleable.CustomMenuItemView_title)
      binding.mtvSubTitle.text = getString(R.styleable.CustomMenuItemView_subTitle)
      binding.mtvSelectedKey.text = getString(R.styleable.CustomMenuItemView_selectedKey)
      recycle()
    }
    binding.mtvSubTitle.visibleIf { !text.isNullOrBlank() }
    if (binding.mtvSelectedKey.text.isNullOrBlank()) {
      binding.mtvSelectedKey.visibility = GONE
    } else {
      binding.mtvSelectedKey.visibility = VISIBLE
    }
  }

  fun selectedValue() =
    if (negativeSelectedValue) {
      -allOptionsMap[binding.mtvSelectedKey.text]!!
    } else {
      allOptionsMap[binding.mtvSelectedKey.text]!!
    }
...
*/
  ...
  fun setSelectedValue(value: Int) {
    binding.mtvSelectedKey.text = allOptionsMap.getKeyByValue(value)
    when (customMenuItemViewType) {
